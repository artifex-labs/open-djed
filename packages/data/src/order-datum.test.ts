import { Data } from "@liqwid-labs/lucid"
import { expect, test } from "vitest"
import { OrderDatum } from "./order-datum"

test('order datum from', () => {
  expect(Data.from(
    '',
    OrderDatum,
  )).toEqual({
    actionFields: {
      BurnDJED: {
        djedAmount: 1000000000n,
      },
    },
    address: {
      paymentKeyHash: [
        '7a3a250a9ec1b52fb4811ad0aa7a62df0a9667daf7a9bea3248a371a',
      ],
      stakeKeyHash: [[['b652ad81db60f5d8605dcec6e73d1ed932009651d9f3e8580ee07d22']]],
    },
    adaExchangeRate: {
      denominator: 10000n,
      numerator: 6309n,
    },
    creationDate: 1744230983000n,
    orderStateTokenMintingPolicyId: '04ea363a127872366ef2d3186325a25a5cee8826ff8a79dc7c8fa671',
  } as OrderDatum)
})

test('order datum to', () => {
  expect(Data.to({
    actionFields: {
      BurnDJED: {
        djedAmount: 1000000000n,
      },
    },
    address: {
      paymentKeyHash: [
        '7a3a250a9ec1b52fb4811ad0aa7a62df0a9667daf7a9bea3248a371a',
      ],
      stakeKeyHash: [[['b652ad81db60f5d8605dcec6e73d1ed932009651d9f3e8580ee07d22']]],
    },
    adaExchangeRate: {
      denominator: 10000n,
      numerator: 6309n,
    },
    creationDate: 1744230983000n,
    orderStateTokenMintingPolicyId: '04ea363a127872366ef2d3186325a25a5cee8826ff8a79dc7c8fa671',
  },
    OrderDatum),
  ).toEqual('')
})